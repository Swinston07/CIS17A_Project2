 ...
 * Created on June 2, 2023, 5:47 PM
 *
...
#include "Player.h"    // Random number generator
        ...
        switch(choice) {
            case '1': { cout << endl << "Lets Play" << endl;
                    do {
                        ...
                        plCalls.calls12();
                        }while(plCalls.gCall1() < 1 || plCalls.gCall2() > 6);
                        
                        //Players make their  first calls and game continues
                        do {
                            cout << p1 <<" says " << plCalls.gCall1() << "," 
                                 << plCalls.gCall2() << endl;
                        ...
                        // Test to see if Player 1 is a liar
                                if (pLiar.getLiar() == 'l' || pLiar.getLiar() == 'L') {
                                    isLiar = pLiar.tOrF1(pCall[0].getDice(), 
                                                         pCall[1].getDice(),
                                                         dice, plCalls.gCall1(),
                                                         plCalls.gCall2());
                        ...
                        // Code for if Player 2 doesn't call Player 1 a liar
                                ...
                                do{
                                    ...
                                    cout << "Player 2 make your call" << endl;
                                    plCalls.calls34();
                                }while(plCalls.gCall3() <= plCalls.gCall1() &&
                                           plCalls.gCall4() <= plCalls.gCall2() || 
                                           plCalls.gCall3() < plCalls.gCall1()  || 
                                           plCalls.gCall4() > 6);
                                
                                    cout << p2 <<" says " << plCalls.gCall3() 
                                         << "," 
                                         << plCalls.gCall4() << endl;
                        ...
                        // If Player 1 calls player 2 a liar
                                    if (pLiar.getLiar() == 'l' ||
                                        ...
                                        isLiar = pLiar.tOrF2(pCall[0].getDice(), 
                                                             pCall[1].getDice(), 
                                                             dice, plCalls.gCall3(),
                                                             plCalls.gCall4());
                                     ...
                                     * 2 a liar*/
                                        ...
                                        do{
                                            ...
                                            plCalls.calls12();
                                        }while(plCalls.gCall1() <= plCalls.gCall3() && 
                                               plCalls.gCall2() <= plCalls.gCall4() ||
                                               plCalls.gCall1() <  plCalls.gCall3() || 
                                               plCalls.gCall2() > 6);
                                    }
                                }
...
bool PlayerLiar::tOrF2(int *a, int *b, int dice, int c1, int c2){
    int count = 0;
...
bool PlayerLiar::tOrF1(int *a, int *b, int dice, int c1, int c2){
    int count = 0;
...
char PlayerLiar::getLiar(){
    return liar;
}

void PlayerLiar::inLiar(){
        cin >> liar;
}
...
Player::Player(){
    plyrDce = new int[NUMDCE];
    
    dce.open("dice.bin",ios::in|ios::binary);
    for(int i = 0; i < NUMPLYR; i++){
        dce.read(reinterpret_cast<char *>(plyrDce),
                (NUMDCE)*sizeof(plyrDce));
    }    
}
